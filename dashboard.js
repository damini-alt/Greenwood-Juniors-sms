...
const dashboard = {
    ...
    supabaseUrl: (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_SUPABASE_URL) ? import.meta.env.VITE_SUPABASE_URL : (window.appConfig?.supabaseUrl || ''),
    supabaseKey: (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_SUPABASE_KEY) ? import.meta.env.VITE_SUPABASE_KEY : (window.appConfig?.supabaseKey || ''),
    ...
    updateDivisionsForSelect: function (classSelectId, divSelectId, includeAllOption = false) {
        const classSelect = document.getElementById(classSelectId);
        const divSelect = document.getElementById(divSelectId);
        if (!classSelect || !divSelect) return;

        const classVal = classSelect.value;
        const divs = this.classDivisionsMap[classVal] || [];

        let optionsHtml = '';
        if (includeAllOption) {
            optionsHtml += '<option value="">All Divisions</option>';
        } else {
            optionsHtml += '<option value="">Select Division</option>';
        }

        optionsHtml += divs.map(d => `<option value="${d}">${d}</option>`).join('');
    ...
    init: async function () {
        ...
        setTimeout(() => this.applyCustomLogo(), 100);
    },
    ...
    loadPage: function (id) {
        ...
        // Apply custom logo to newly rendered content
        setTimeout(() => this.applyCustomLogo(), 0);
    },
    ...
    submitAttendance: async function () {
        ...
        if (auth.currentUser.role === 'staff' || auth.currentUser.role === 'admin' || auth.currentUser.role === 'teacher') {
            try {
                ...
                const allRecords = attendanceData.map(d => {
                ...
                const absentOnly = allRecords.filter(r => r.status !== 'Present');

                if (absentOnly.length > 0) {
                    const webhookUrl = window.appConfig?.webhooks?.attendance || 'https://studio.pucho.ai/api/v1/webhooks/Ug2Ek52i7YXsYIOAyId9H';
    ...
    saveExamMarks: async function () {
        ...
        if (this.isDbConnected) {
            ...
            if (result) {
                ...
                try {
                    const webhookUrl = window.appConfig?.webhooks?.reportCard || 'https://studio.pucho.ai/api/v1/webhooks/6eOOY4DZDtZ4qme9OQo9s';
    ...
    submitStaffData: async function (event) {
        ...
        const hra = parseFloat(document.getElementById('staffHra').value) || 0;
        const conveyance = parseFloat(document.getElementById('staffConveyance').value) || 0;
        const specialAllowance = parseFloat(document.getElementById('staffSpecialAllowance').value) || 0;
        ...
        if (this.isDbConnected) {
            try {
                ...
                const webhookUrl = window.appConfig?.webhooks?.staffOnboarding || 'https://studio.pucho.ai/api/v1/webhooks/2EYTPsOBUHLc3o4MEzL9M';
                fetch(webhookUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    editStaff: function (id) {
        ...
        document.getElementById('staffHra').value = staff.hra || 0;
        document.getElementById('staffConveyance').value = staff.conveyance || 0;
        document.getElementById('staffSpecialAllowance').value = staff.special_allowance || 0;
    ...
    publishNotice: async function () {
        ...
        // Dispatch Webhook
        try {
            const webhookUrl = window.appConfig?.webhooks?.notice || 'https://studio.pucho.ai/api/v1/webhooks/a46uCRwjp7Q0Bp9XwDkjG';
    ...
    scheduleExam: async function () {
        ...
        try {
            ...
            const webhookUrl = window.appConfig?.webhooks?.examSchedule || "https://studio.pucho.ai/api/v1/webhooks/5ehEmQ2nUP6a2gq2m0WlX";
    ...
    submitStudentData: async function (e) {
        ...
        const bloodGroup = document.getElementById('stdBloodGroup').value;
        const allergies = document.getElementById('stdAllergies').value;
        const doctorName = document.getElementById('stdDoctorName').value;
        const emergencyContact = document.getElementById('stdEmergencyContact').value;
        const medicalNotes = document.getElementById('stdMedicalNotes').value;
    ...
    editStudent: function (id) {
        ...
        document.getElementById('stdBloodGroup').value = student.blood_group || 'O+';
        document.getElementById('stdAllergies').value = student.allergies || 'None';
        document.getElementById('stdDoctorName').value = student.doctor_name || '';
        document.getElementById('stdEmergencyContact').value = student.emergency_contact || '';
        document.getElementById('stdMedicalNotes').value = student.medical_notes || '';
    ...
    students: function () {
        ...
        setTimeout(() => {
            ...
            <tr class="hover:bg-gray-50/50 transition-all">
                <td class="p-6 border-b border-gray-50">
                    <div class="font-bold text-pucho-dark flex items-center gap-2">
                        ${s.name}
                        ${(s.allergies && s.allergies !== 'None' && s.allergies !== 'none') || s.medical_notes ? 
                            `<span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-50 text-red-500 text-[10px] cursor-help" title="Health Alert: ${s.allergies || s.medical_notes || 'Allergy/medical warning'}">🩺</span>` : ''
                        }
                    </div>
        ...
        return `<div id="studentModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-pucho-dark/40 backdrop-blur-sm hidden animate-fade-in">
                <div class="bg-white p-8 w-full max-w-2xl rounded-[32px] border border-white/30 shadow-2xl relative animate-slide-up max-h-[90vh] overflow-y-auto">
                    ...
                    <form id="studentForm" class="space-y-6 font-inter">
                        ...
                        <!-- 4. Health & Medical Details -->
                        <div>
                            <h4 class="text-sm font-bold text-pucho-purple uppercase tracking-widest mb-4">Health & Medical Details</h4>
                            <div class="grid grid-cols-2 gap-4">
                                <div><label class="label-sm">Blood Group</label><select id="stdBloodGroup" class="input-field"><option>A+</option><option>A-</option><option>B+</option><option>B-</option><option>AB+</option><option>AB-</option><option>O+</option><option>O-</option></select></div>
                                <div><label class="label-sm">Allergies</label><input type="text" id="stdAllergies" class="input-field" placeholder="e.g. Peanuts, Dairy, None"></div>
                                <div><label class="label-sm">Pediatrician / Doctor</label><input type="text" id="stdDoctorName" class="input-field" placeholder="e.g. Dr. Patil"></div>
                                <div><label class="label-sm">Emergency Contact</label><input type="tel" id="stdEmergencyContact" class="input-field" placeholder="+91 98765 43210"></div>
                                <div class="col-span-2"><label class="label-sm">Medical Notes / Warnings</label><textarea id="stdMedicalNotes" class="input-field h-16 resize-none" placeholder="Enter any specific medical notes or warnings..."></textarea></div>
                            </div>
                        </div>

                        <div class="pt-8 border-t border-gray-50 flex justify-end items-center gap-6">
                            <button type="button" onclick="document.getElementById('studentModal').classList.add('hidden')" class="text-sm font-bold text-gray-400 hover:text-pucho-dark transition-colors">Discard Changes</button>
    ...
    staff: function () {
        if (!this.staffActiveTab) this.staffActiveTab = 'directory';
        ...
        // Initial load rows
        if (this.staffActiveTab === 'payroll') {
            setTimeout(() => dashboard.filterStaffPayroll(), 0);
        } else {
            setTimeout(() => dashboard.filterStaff(), 0);
        }

        const activeTabClass = "bg-pucho-dark text-white shadow-lg";
        const inactiveTabClass = "text-gray-400 hover:text-pucho-dark hover:bg-gray-50";

        const tabsHtml = `
            <div class="flex gap-2 p-1 bg-gray-50 rounded-2xl w-fit mb-8 font-inter">
                <button onclick="dashboard.switchStaffTab('directory')" class="px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${this.staffActiveTab === 'directory' ? activeTabClass : inactiveTabClass}">
                    📋 Directory
                </button>
                <button onclick="dashboard.switchStaffTab('payroll')" class="px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${this.staffActiveTab === 'payroll' ? activeTabClass : inactiveTabClass}">
                    💰 Payroll & Salaries
                </button>
            </div>
        `;
        ...
        if (this.staffActiveTab === 'payroll') {
            return `<div class="space-y-8 animate-fade-in">
                    ${tabsHtml}
                    <div class="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-subtle animate-fade-in">
                        <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div>
                                <h3 class="font-bold text-2xl">Staff Payroll Dashboard</h3>
                                <p class="text-gray-400 text-sm">Monthly Compensation & EPF calculations</p>
                            </div>
                            ...
                            <table class="w-full text-left font-inter">
                                <thead class="bg-gray-50/50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Gross Earnings</th>
                                        <th class="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Deductions</th>
                                        <th class="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Net Salary</th>
                                        <th class="px-6 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status</th>
                    ...
                    <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div class="flex gap-4 items-center">
                             <select id="filterClass" onchange="dashboard.filterStaff()" class="px-4 py-2 rounded-xl border border-gray-200 text-sm font-bold text-gray-600 outline-none focus:border-pucho-purple">
                                <option value="">All Classes</option><option value="Playgroup">Playgroup</option><option value="Nursery">Nursery</option><option value="LKG">LKG</option><option value="UKG">UKG</option>
                            </select>
                    ...
                    <form id="staffForm" class="space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div class="col-span-1 md:col-span-2 border-t border-gray-100 pt-4 mt-2">
                                <h4 class="text-sm font-bold text-pucho-purple uppercase tracking-widest mb-4">Compensation & Salary Setup</h4>
                            </div>
                            <div class="col-span-1">
                                <label class="${labelClass}">Basic Salary (Monthly)</label>
                                <input type="number" id="staffBasicSalary" class="${inputClass}" placeholder="e.g. 25000" required>
                            </div>
                            <div class="col-span-1">
                                <label class="${labelClass}">House Rent Allowance (HRA)</label>
                                <input type="number" id="staffHra" class="${inputClass}" placeholder="e.g. 5000" required>
                            </div>
                            <div class="col-span-1">
                                <label class="${labelClass}">Conveyance Allowance</label>
                                <input type="number" id="staffConveyance" class="${inputClass}" placeholder="e.g. 1500" required>
                            </div>
                            <div class="col-span-1">
                                <label class="${labelClass}">Special Allowance</label>
                                <input type="number" id="staffSpecialAllowance" class="${inputClass}" placeholder="e.g. 2000" required>
                            </div>
    ...
    communication: function (role) {
        ...
        let noticeRows = notices.map(n => {
            const isEvent = n.title && (n.title.toLowerCase().includes('event') || n.title.toLowerCase().includes('annual') || n.title.toLowerCase().includes('ptm') || n.title.toLowerCase().includes('day'));
            const rsvpKey = `rsvp_${n.id}_${auth.currentUser?.id || 'user'}`;
            const rsvped = localStorage.getItem(rsvpKey);
            return `
                <div class="bg-white p-6 rounded-[32px] border border-gray-100 shadow-subtle flex flex-col gap-4 animate-fade-in hover:shadow-glow transition-all group">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-[10px] font-bold uppercase tracking-widest">${n.target || 'All'}</span>
                                ${n.priority === 'High' ? '<span class="px-3 py-1 bg-red-100 text-red-600 rounded-full text-[10px] font-bold">🔴 High Priority</span>' : ''}
                                ${isEvent ? '<span class="px-3 py-1 bg-amber-100 text-amber-600 rounded-full text-[10px] font-bold">📅 Event</span>' : ''}
                            </div>
                    ...
                    <div class="pt-4 border-t border-gray-50 flex justify-between items-center flex-wrap gap-3">
                        <span class="text-xs font-bold text-gray-400">By Admin</span>
                        <div class="flex items-center gap-2">
                            ${role === 'admin' ? `
                                <a href="https://wa.me/?text=${encodeURIComponent('📢 ' + n.title + '\n\n' + n.content + '\n\n- School Admin')}" target="_blank"
                                    class="flex items-center gap-1.5 bg-[#25D366] text-white px-4 py-2 rounded-full text-xs font-bold hover:scale-105 transition-all">
                            ...
                            ${isEvent && role === 'parent' ? `
                                <button onclick="dashboard.rsvpNotice('${n.id}', this)" 
                                    class="px-4 py-2 rounded-full text-xs font-bold transition-all ${rsvped === 'yes' ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-pucho-purple/10 text-pucho-purple hover:bg-pucho-purple hover:text-white'}">
                                    ${rsvped === 'yes' ? '✅ Acknowledged' : '👋 RSVP / Acknowledge'}
                                </button>` : ''}
                        </div>
                    </div>
                </div>
        ...
        return `<div class="space-y-8 animate-fade-in">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-400 text-sm">Circulars, events & parent notifications</p>
                    </div>
                    ${role === 'admin' ? `
                    <div class="flex items-center gap-3">
                        <a href="https://wa.me/?text=${encodeURIComponent('📢 School Broadcast\n\nDear Parents,\n\nPlease check the latest notice on our parent portal.\n\n- School Admin')}" target="_blank"
                            class="flex items-center gap-2 bg-[#25D366] text-white px-5 py-2.5 rounded-full text-sm font-bold hover:scale-105 transition-all shadow-md">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            WA Broadcast
                        </a>
                        <button onclick="dashboard.showBroadcastModal()" class="bg-pucho-dark text-white px-6 py-2.5 rounded-2xl font-bold hover:shadow-glow hover:bg-pucho-purple transition-all text-sm">+ New Post</button>
                    </div>` : ''}
                </div>
    ...
    rsvpNotice: function(noticeId, btn) {
        const rsvpKey = `rsvp_${noticeId}_${auth.currentUser?.id || 'user'}`;
        localStorage.setItem(rsvpKey, 'yes');
        btn.innerHTML = '✅ Acknowledged';
        btn.className = 'px-4 py-2 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-300';
    ...
    parent_notices: function () {
        ...
        const noticesHtml = this.renderNoticeList(notices, 'School Announcements');
        const eventsHtml = this.renderEventsList();
    ...
    renderEventsList: function () {
        const events = schoolDB.events || [];
        const userId = window.auth?.currentUser?.id || 'user';
        
        const eventCards = events.map(evt => {
            const rsvpKey = `event_rsvp_${evt.id}_${userId}`;
            const userChoice = localStorage.getItem(rsvpKey); // 'attending' or 'not_attending' or null
            
            // Adjust RSVP count dynamically
            let currentCount = evt.rsvpCount || 0;
            if (userChoice === 'attending') {
                currentCount += 1;
            }
            
            const isAttending = userChoice === 'attending';
            const isNotAttending = userChoice === 'not_attending';
            
            return `
                <div class="bg-white p-6 rounded-[32px] border border-gray-100 shadow-subtle flex flex-col gap-4 hover:shadow-glow transition-all group overflow-hidden">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-[10px] font-bold uppercase tracking-widest">📅 Event</span>
                                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">${evt.date} • ${evt.time}</span>
                            </div>
                            <h4 class="text-xl font-bold text-pucho-dark group-hover:text-pucho-purple transition-colors">${evt.title}</h4>
                        </div>
                    </div>
                    <p class="text-gray-600 text-sm leading-relaxed">${evt.description || ''}</p>
                    <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 bg-gray-50 p-3 rounded-2xl">
                        <span>📍 Venue: <strong>${evt.venue || 'School Campus'}</strong></span>
                    </div>
                    <div class="pt-4 border-t border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <span class="text-xs font-bold text-purple-600 bg-purple-50 px-3 py-1.5 rounded-full">
                            👥 ${currentCount} Attending
                        </span>
                        <div class="flex gap-2 w-full sm:w-auto">
                            <button onclick="dashboard.rsvpEvent('${evt.id}', 'attending')" 
                                class="flex-1 sm:flex-initial px-4 py-2 rounded-full text-xs font-bold transition-all ${isAttending ? 'bg-green-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-green-50 hover:text-green-600'}">
                                Attending
                            </button>
                            <button onclick="dashboard.rsvpEvent('${evt.id}', 'not_attending')" 
                                class="flex-1 sm:flex-initial px-4 py-2 rounded-full text-xs font-bold transition-all ${isNotAttending ? 'bg-red-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-red-50 hover:text-red-600'}">
                                Not Attending
                            </button>
                        </div>
                    </div>
        ...
        return `
            <div class="space-y-6 font-inter">
                <div>
                    <h3 class="font-bold text-2xl text-pucho-dark">Upcoming School Events</h3>
                    <p class="text-gray-400 text-sm">Interactive RSVPs for upcoming preschool events</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ${eventCards || '<p class="text-gray-400 font-bold col-span-2 text-center py-10 italic">No upcoming events scheduled.</p>'}
                </div>
            </div>
        `;
    },

    rsvpEvent: function (eventId, choice) {
        const userId = window.auth?.currentUser?.id || 'user';
        const rsvpKey = `event_rsvp_${eventId}_${userId}`;
        localStorage.setItem(rsvpKey, choice);
        
        const msg = choice === 'attending' ? 'You are marked as Attending! 🌟' : 'You are marked as Not Attending.';
        showToast(msg, 'success');
        
        // POST to Parent Portal RSVP Webhook
        try {
            const webhookUrl = window.appConfig?.webhooks?.parentPortal || 'https://studio.pucho.ai/api/v1/webhooks/F5M67RflZnA9nHe02EFeG';
            fetch(webhookUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    parent_id: userId,
                    status: choice,
                    comments: choice === 'attending' ? 'Attending' : 'Not Attending'
                })
            }).then(res => {
                if (res.ok) console.log('[rsvpEvent] Webhook success');
                else console.error('[rsvpEvent] Webhook status:', res.status);
            }).catch(e => console.error('[rsvpEvent] Webhook error:', e));
        } catch (e) {
            console.error('[rsvpEvent] Webhook throw:', e);
        }
        
        // Refresh view
        const content = this.parent_notices();
    ...
    renderNoticeList: function (notices, title) {
        const cards = notices.map(n => {
            ...
            const rsvpKey = `rsvp_${n.id}_${window.auth?.currentUser?.id || 'user'}`;
            const rsvped = localStorage.getItem(rsvpKey) === 'yes';
            ...
            return `
                <div class="bg-white p-6 rounded-[32px] border border-gray-100 shadow-subtle flex flex-col gap-4 animate-fade-in hover:shadow-glow transition-all group overflow-hidden">
                    ...
                    <div class="pt-4 border-t border-gray-50 flex justify-between items-center flex-wrap gap-2">
                        <span class="text-xs font-bold text-gray-400">By Admin</span>
                        ${n.rsvp ? `
                            <button onclick="dashboard.rsvpNotice('${n.id}', this)" 
                                class="px-4 py-2 rounded-full text-xs font-bold transition-all ${rsvped ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-pucho-purple/10 text-pucho-purple hover:bg-pucho-purple hover:text-white'}">
                                ${rsvped ? '✅ Acknowledged' : '👋 Acknowledge'}
                            </button>
                        ` : ''}
                    </div>
                </div>`;
        ...
        return `<div class="space-y-8 animate-fade-in font-inter">
                ...
                <div class="bg-white p-8 rounded-[40px] border border-gray-100 shadow-subtle">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h4 class="font-bold text-lg">Management Automations</h4>
                            <p class="text-xs text-gray-400 mt-1">Trigger school-wide administrative workflows directly from the dashboard</p>
                        </div>
                        <span class="text-[10px] font-black text-pucho-purple uppercase tracking-widest bg-pucho-purple/5 px-3 py-1 rounded-full">Pucho Studio Flow</span>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <button onclick="dashboard.triggerManagementFlow('homework_broadcast')" class="flex flex-col items-start p-6 rounded-3xl border border-gray-100 hover:border-pucho-purple hover:bg-pucho-purple/5 transition-all text-left group">
                            <span class="text-2xl mb-3">📚</span>
                            <span class="font-bold text-sm text-pucho-dark group-hover:text-pucho-purple transition-all">Homework Broadcast</span>
                            <span class="text-xs text-gray-400 mt-1">Notify all parents about latest homework updates</span>
                        </button>
                        <button onclick="dashboard.triggerManagementFlow('salary_calculation')" class="flex flex-col items-start p-6 rounded-3xl border border-gray-100 hover:border-pucho-purple hover:bg-pucho-purple/5 transition-all text-left group">
                            <span class="text-2xl mb-3">💸</span>
                            <span class="font-bold text-sm text-pucho-dark group-hover:text-pucho-purple transition-all">Calculate Salaries</span>
                            <span class="text-xs text-gray-400 mt-1">Process current month's payroll & email payslips</span>
                        </button>
                        <button onclick="dashboard.triggerManagementFlow('fee_reminder')" class="flex flex-col items-start p-6 rounded-3xl border border-gray-100 hover:border-pucho-purple hover:bg-pucho-purple/5 transition-all text-left group">
                            <span class="text-2xl mb-3">💳</span>
                            <span class="font-bold text-sm text-pucho-dark group-hover:text-pucho-purple transition-all">Send Fee Reminders</span>
                            <span class="text-xs text-gray-400 mt-1">Send outstanding fee notices to pending parents</span>
                        </button>
                    </div>
                </div>
    ...
    settings: function () {
        ...
        if (user.role === 'admin' || user.role === 'staff') {
            if (user.role === 'admin') {
                content += `<div class="bg-white rounded-[40px] p-10 border border-gray-100 shadow-subtle font-inter mb-10">
                    <h3 class="font-black text-3xl text-pucho-dark mb-8 tracking-tight">School Branding</h3>
                    <div class="flex flex-col md:flex-row items-center gap-8 bg-gray-50 p-8 rounded-[32px] border border-gray-100">
                        <div class="w-48 h-48 bg-white rounded-3xl border-2 border-dashed border-gray-200 flex items-center justify-center relative overflow-hidden group">
                            <img id="settingsSchoolLogoPreview" src="${localStorage.getItem('pucho_school_logo') || '/images/pucho_logo_login.png'}" class="max-w-full max-h-full object-contain p-4 transition-transform group-hover:scale-105">
                            <div id="schoolLogoLoadingOverlay" class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center opacity-0 transition-opacity z-10">
                                <div class="w-8 h-8 border-4 border-pucho-purple border-t-transparent rounded-full animate-spin mb-2"></div>
                                <span class="text-[10px] font-black uppercase tracking-widest text-pucho-purple">Uploading</span>
                            </div>
                        </div>
                        <div class="flex-1">
                            <h4 class="font-black text-xl text-pucho-dark mb-2">Custom School Logo</h4>
                            <p class="text-sm text-gray-500 font-medium mb-6">Upload your institution's logo to customize the application branding. Recommended: PNG, SVG, or PDF (max 2MB).</p>
                            <label class="bg-pucho-dark text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-pucho-purple transition-all shadow-glow cursor-pointer inline-flex items-center gap-2 active:scale-95">
                                <span>Upload Image</span>
                                <input type="file" class="hidden" accept=".png, .jpg, .jpeg, .svg, .pdf" onchange="dashboard.handleSchoolLogoUpload(this)">
                            </label>
                        </div>
                    </div>
                </div>`;
    ...
    trimImage: function (imageSource, callback) {
        const img = new Image();
        ...
        img.onload = function () {
            ...
            try {
                ...
                if (transparentPixels > (data.length / 4) * 0.01) { // More than 1% of pixels are transparent
                ...
                let cornersAreWhite = true;
                corners.forEach(idx => {
                    const r = data[idx];
                    const g = data[idx+1];
                    const b = data[idx+2];
                    const a = data[idx+3];
                    if (a < 50) return; // Transparent corner is fine
                    if (r < 240 || g < 240 || b < 240) {
                        cornersAreWhite = false;
                    }
                });

                const isBg = (r, g, b, a) => {
                    if (a < 15) return true; // Transparent is always background
                    if (!hasTransparency && cornersAreWhite) {
                        // If there is no transparency and corners are white, then white is background
                ...
                if (!foundAny) {
                    // If image is blank or pure white/transparent, don't crop
                    callback(imageSource);
                    return;
                }
                
                // Add small padding (2% of width/height or at least 4px) to prevent cutting edges
                const pad = Math.max(4, Math.floor(Math.min(width, height) * 0.015));
                ...
                console.error("Error trimming image margins:", e);
    ...
    handleSchoolLogoUpload: function (input) {
        if (!input.files || !input.files[0]) return;
        const file = input.files[0];
        ...
        if (!validTypes.includes(file.type)) {
            showToast('Invalid file type. Please upload PNG, JPG, SVG, or PDF.', 'error');
            return;
        }
        ...
        const overlay = document.getElementById('schoolLogoLoadingOverlay');
        if (overlay) overlay.style.opacity = '1';

        const saveAndApplyLogo = (base64, msg) => {
            localStorage.setItem('pucho_school_logo', base64);
            localStorage.setItem('pucho_logo_trimmed', 'true');
            const preview = document.getElementById('settingsSchoolLogoPreview');
        ...
        if (file.type === 'application/pdf') {
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    this.trimImage(rawBase64, (trimmedBase64) => {
                        saveAndApplyLogo(trimmedBase64, 'School logo updated from PDF!');
                    });
            ...
            reader.onload = (e) => {
                ...
                this.trimImage(rawBase64, (trimmedBase64) => {
                    saveAndApplyLogo(trimmedBase64, 'School logo updated successfully!');
                });
            };
            reader.onerror = () => {
                if (overlay) overlay.style.opacity = '0';
    ...
    applyCustomLogo: function () {
        const customLogo = localStorage.getItem('pucho_school_logo');
        if (customLogo) {
            ...
            // Automatically trim the existing logo if it hasn't been trimmed yet
            const isTrimmed = localStorage.getItem('pucho_logo_trimmed');
            if (!isTrimmed) {
                this.trimImage(customLogo, (trimmedLogo) => {
                    localStorage.setItem('pucho_school_logo', trimmedLogo);
                    localStorage.setItem('pucho_logo_trimmed', 'true');
                    ...
                    const preview = document.getElementById('settingsSchoolLogoPreview');
                    if (preview) preview.src = trimmedLogo;
                });
            }
        }
    },
        ...
        return `<div class="animate-fade-in space-y-8">
                ...
                <div class="bg-white rounded-[40px] p-8 md:p-12 border border-gray-100 shadow-subtle relative overflow-hidden">
                    ...
                    <div class="flex flex-col md:flex-row gap-10 items-center relative z-10">
                        <div class="flex-none text-center">
                            <div class="w-40 h-40 bg-amber-50 rounded-full border-4 border-white shadow-xl mx-auto mb-4 flex items-center justify-center text-6xl">👧</div>
                            <span class="bg-green-100 text-green-700 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest">Active</span>
                        </div>
                        <div class="flex-1 text-center md:text-left">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase mb-1">Roll No.</p>
                                    <p class="font-bold text-lg text-pucho-dark">#${student.roll_no || student.rollNo || '01'}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase mb-1">Blood Group</p>
                                    <p class="font-bold text-lg text-red-500">${student.blood_group || 'O+'}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase mb-1">Date of Birth</p>
                                    <p class="font-bold text-lg text-pucho-dark">${student.dob || '12 Aug 2020'}</p>
                                </div>
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase mb-1">Parent Contact</p>
                                    <p class="font-bold text-base text-pucho-purple">${student.phone || '9876543210'}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <h3 class="font-bold text-xl text-pucho-dark">Health Information</h3>
                    </div>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div class="bg-red-50/50 p-4 rounded-2xl border border-red-100">
                            <p class="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-1">Blood Group</p>
                            <p class="font-bold text-pucho-dark text-lg">${student.blood_group || 'O+'}</p>
                        </div>
                        <div class="bg-amber-50/50 p-4 rounded-2xl border border-amber-100">
                            <p class="text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-1">Allergies</p>
                            <p class="font-bold text-pucho-dark text-sm">${student.allergies || 'None reported'}</p>
                        </div>
                        <div class="bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
                            <p class="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">Doctor</p>
                            <p class="font-bold text-pucho-dark text-sm">${student.doctor_name || 'Dr. Sharma'}</p>
                        </div>
                        <div class="bg-green-50/50 p-4 rounded-2xl border border-green-100">
                            <p class="text-[10px] font-bold text-green-500 uppercase tracking-widest mb-1">Emergency</p>
                            <p class="font-bold text-pucho-dark text-sm">${student.emergency_contact || student.phone || '9876543210'}</p>
                        </div>
                    </div>
                    ${student.medical_notes ? `<div class="mt-4 p-4 bg-red-50 border border-red-100 rounded-2xl"><p class="text-xs font-bold text-red-600 uppercase tracking-widest mb-1">⚠️ Medical Notes</p><p class="text-sm text-red-700">${student.medical_notes}</p></div>` : ''}
                </div>

                <!-- Daily Activity Log -->
                <div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle">
                    <div class="flex items-center gap-3 mb-6">
                        <div class="w-10 h-10 rounded-2xl bg-amber-50 flex items-center justify-center text-xl">🌞</div>
                        <h3 class="font-bold text-xl text-pucho-dark">Today's Activity Log</h3>
                        <span class="ml-auto text-xs font-bold text-gray-400">${new Date().toLocaleDateString('en-IN', {weekday:'long', day:'numeric', month:'long'})}</span>
                    </div>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="text-center p-4 bg-amber-50 rounded-[20px] border border-amber-100">
                            <div class="text-3xl mb-2">🍱</div>
                            <p class="text-[10px] font-bold uppercase tracking-widest text-amber-600 mb-1">Meals</p>
                            <p class="text-sm font-bold text-pucho-dark">Ate Well</p>
                        </div>
                        <div class="text-center p-4 bg-blue-50 rounded-[20px] border border-blue-100">
                            <div class="text-3xl mb-2">😴</div>
                            <p class="text-[10px] font-bold uppercase tracking-widest text-blue-500 mb-1">Nap Time</p>
                            <p class="text-sm font-bold text-pucho-dark">1.5 hrs</p>
                        </div>
                        <div class="text-center p-4 bg-green-50 rounded-[20px] border border-green-100">
                            <div class="text-3xl mb-2">🌟</div>
                            <p class="text-[10px] font-bold uppercase tracking-widest text-green-600 mb-1">Mood</p>
                            <p class="text-sm font-bold text-pucho-dark">Happy</p>
                        </div>
                        <div class="text-center p-4 bg-purple-50 rounded-[20px] border border-purple-100">
                            <div class="text-3xl mb-2">🎨</div>
                            <p class="text-[10px] font-bold uppercase tracking-widest text-purple-600 mb-1">Activity</p>
                            <p class="text-sm font-bold text-pucho-dark">Art & Craft</p>
                        </div>
                    </div>
                </div>

                <!-- Performance Analytics -->
                <h3 class="text-xl font-bold ml-2">📈 Progress Analytics</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ${createBarGraph('Attendance Trends (6 Months)', ['Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan'], attendanceData, 'bg-pucho-purple')}
                    ${createBarGraph('Activity Performance', ['Art', 'Music', 'Math', 'Sci', 'Reading'], marksData, 'bg-amber-400')}
                </div>
                
                <!-- Teacher Remarks -->
                <div class="bg-gray-50 rounded-[40px] p-8 border border-gray-100">
                    <h3 class="font-bold text-xl mb-6">💬 Teacher's Remarks</h3>
                    <div class="space-y-4">
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 flex gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1">"${student.name} is showing wonderful progress in arts and social skills. Very curious and enthusiastic every day!"</p>
                                <p class="text-[10px] font-bold text-gray-400 mt-2">Today</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    my_fees: function () {
        ...
        const totalPaid = history.filter(f => f.status === 'Paid').reduce((acc, f) => acc + f.amount, 0);

        let t1Amt = 0;
        let t2Amt = 0;
        myStudents.forEach(student => {
            const cls = student.class || 'Nursery';
            const structure = schoolDB.schoolInfo.feeStructure[cls] || { term1: 20000, term2: 20000 };
            t1Amt += structure.term1;
            t2Amt += structure.term2;
        });

        const t1Pending = history.some(f => f.type.toLowerCase().includes('term 1') && f.status === 'Pending');
        const t1Paid = history.some(f => f.type.toLowerCase().includes('term 1') && f.status === 'Paid');
        const term1Status = t1Pending ? 'Pending' : (t1Paid ? 'Paid' : 'Pending');

        const t2Pending = history.some(f => f.type.toLowerCase().includes('term 2') && f.status === 'Pending');
        const t2Paid = history.some(f => f.type.toLowerCase().includes('term 2') && f.status === 'Paid');
        const term2Status = t2Pending ? 'Pending' : (t2Paid ? 'Paid' : 'Upcoming');

        // Biannual structure
        const term1 = { label: 'Term 1 (April – September)', amount: t1Amt, status: term1Status, dueDate: '2025-04-10' };
        const term2 = { label: 'Term 2 (October – March)', amount: t2Amt, status: term2Status, dueDate: '2025-10-10' };

        const rows = history.map(f => `
            <div class="flex justify-between items-center bg-white p-5 rounded-2xl border border-gray-100 hover:shadow-md transition-shadow group">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-xl ${f.status === 'Paid' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'} flex items-center justify-center text-lg">💰</div>
                    <div>
                        <p class="font-bold text-sm text-pucho-dark">${f.type} — <span class="text-pucho-purple">${f.student}</span></p>
                        <p class="text-xs text-gray-400 font-bold">${f.status === 'Paid' ? '✅ Paid on ' + (f.paidDate || f.date) : '⏳ Due ' + f.dueDate}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="font-black ${f.status === 'Paid' ? 'text-green-600' : 'text-red-500'} text-base">₹${f.amount.toLocaleString()}</p>
                    ${f.status === 'Pending' ? `<button onclick="showToast('Redirecting to Payment Gateway...', 'info')" class="text-[10px] font-black uppercase text-pucho-purple hover:underline mt-1 block">Pay Now →</button>` : `<button onclick="showToast('Receipt downloaded!', 'success')" class="text-[10px] font-black uppercase text-gray-400 hover:text-pucho-purple hover:underline mt-1 block">📄 Receipt</button>`}
                </div>
            </div>
        `).join('');
        ...
        return `<div class="animate-fade-in space-y-8">
            ...
            <!-- Outstanding Alert -->
            <div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle text-center relative overflow-hidden">
                <div class="absolute top-0 right-0 w-48 h-48 ${totalPending > 0 ? 'bg-red-50' : 'bg-green-50'} rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl"></div>
                <div class="relative z-10">
                    <div class="w-20 h-20 ${totalPending > 0 ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'} rounded-full flex items-center justify-center text-3xl mx-auto mb-4 shadow-sm">${totalPending > 0 ? '⚠️' : '✅'}</div>
                    <h3 class="text-2xl font-bold text-pucho-dark tracking-tight mb-1">${totalPending > 0 ? 'Outstanding: ₹' + totalPending.toLocaleString() : 'All Dues Cleared! 🎉'}</h3>
                    <p class="text-gray-400 text-sm max-w-sm mx-auto">${totalPending > 0 ? 'Please clear pending dues to avoid late fee.' : 'You have cleared all invoices for this session.'}</p>
                    ${totalPending > 0 ? `
                    <div class="flex items-center justify-center gap-3 mt-6">
                        <button onclick="showToast('Redirecting to Payment Gateway...', 'info')" class="bg-pucho-purple text-white px-8 py-3 rounded-full font-bold hover:shadow-glow transition-all">Pay Now</button>
                        <a href="https://wa.me/919876543210?text=Hi%2C%20I%20have%20a%20fee%20related%20query." target="_blank" class="flex items-center gap-2 bg-[#25D366] text-white px-6 py-3 rounded-full font-bold hover:scale-105 transition-all text-sm">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            Query on WhatsApp
                        </a>
                    </div>` : ''}
            ...
            <!-- Biannual Fee Structure -->
            <div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle">
                <h4 class="font-bold text-sm uppercase text-gray-400 tracking-widest mb-6">📅 Biannual Fee Schedule — 2025–26</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="p-6 rounded-[28px] border-2 ${term1.status === 'Paid' ? 'border-green-200 bg-green-50/50' : 'border-red-200 bg-red-50/50'} relative overflow-hidden">
                        <div class="absolute top-0 right-0 text-6xl opacity-5 font-black">T1</div>
                        <p class="text-[10px] font-bold uppercase tracking-widest ${term1.status === 'Paid' ? 'text-green-600' : 'text-red-500'} mb-2">${term1.status === 'Paid' ? '✅ PAID' : '⚠️ DUE'}</p>
                        <h5 class="font-bold text-pucho-dark text-lg mb-1">Term 1 Installment</h5>
                        <p class="text-xs text-gray-500 mb-3">April – September 2025 · Due: ${term1.dueDate}</p>
                        <p class="text-2xl font-black ${term1.status === 'Paid' ? 'text-green-600' : 'text-red-500'}">₹${term1.amount.toLocaleString()}</p>
                        ${term1.status !== 'Paid' ? `<button onclick="showToast('Payment gateway loading...','info')" class="mt-3 bg-red-500 text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-red-600 transition-all">Pay Term 1</button>` : '<p class="mt-3 text-xs text-green-600 font-bold">Receipt available ↓</p>'}
                    </div>
                    <div class="p-6 rounded-[28px] border-2 ${term2.status === 'Paid' ? 'border-green-200 bg-green-50/50' : (term2.status === 'Pending' ? 'border-red-200 bg-red-50/50' : 'border-gray-100 bg-gray-50/50')} relative overflow-hidden">
                        <div class="absolute top-0 right-0 text-6xl opacity-5 font-black">T2</div>
                        <p class="text-[10px] font-bold uppercase tracking-widest ${term2.status === 'Paid' ? 'text-green-600' : (term2.status === 'Pending' ? 'text-red-500' : 'text-gray-400')} mb-2">
                            ${term2.status === 'Paid' ? '✅ PAID' : (term2.status === 'Pending' ? '⚠️ DUE' : '🔜 UPCOMING')}
                        </p>
                        <h5 class="font-bold text-pucho-dark text-lg mb-1">Term 2 Installment</h5>
                        <p class="text-xs text-gray-500 mb-3">October – March 2026 · Due: ${term2.dueDate}</p>
                        <p class="text-2xl font-black ${term2.status === 'Paid' ? 'text-green-600' : (term2.status === 'Pending' ? 'text-red-500' : 'text-gray-400')}">₹${term2.amount.toLocaleString()}</p>
                        ${term2.status === 'Pending' ? `<button onclick="showToast('Payment gateway loading...','info')" class="mt-3 bg-red-500 text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-red-600 transition-all">Pay Term 2</button>` : (term2.status === 'Paid' ? '<p class="mt-3 text-xs text-green-600 font-bold">Receipt available ↓</p>' : '<p class="mt-3 text-xs text-gray-400 font-bold">Opens October 2025</p>')}
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-2xl mx-auto space-y-3">
                <h4 class="font-bold text-xs uppercase text-gray-400 tracking-widest ml-2">📋 Payment History</h4>
                ${history.length > 0 ? rows : '<div class="py-12 text-center text-gray-400 italic">No payment history found.</div>'}
            </div>
        </div>`;
    },
    ...
    my_classes: function () {
        ...
        const subjects = ['English', 'Rhymes & Stories', 'Free Play', 'General Awareness', 'Art & Craft', 'Nap Time', staff.subject || 'Music & Rhymes'];
        const classes = ['Playgroup-Butterflies', 'Nursery-Rainbows', 'LKG-Daisies', 'UKG-Roses'];
                ...
                <div class="container">
                    <div class="header">
                        <div class="logo-section">
                            <div class="logo">
                                <img src="${window.location.origin}/images/pucho_logo_login.png" alt="Pucho.ai Logo" class="school-logo-img">
                            </div>
                    ...
                    <div class="footer">
                        ...
                        <div class="footer-brand">
                            <div class="footer-logo">
                                <img src="${window.location.origin}/images/pucho_logo_login.png" alt="Pucho.ai" class="school-logo-img">
                            </div>
    ...
    mark_attendance: function () {
        ...
        return `<div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle animate-fade-in font-inter">
                <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <div class="flex gap-4">
                         <div class="relative">
                            <select id="attClass" class="appearance-none bg-gray-50 border border-gray-200 rounded-2xl px-6 py-3 font-bold text-sm outline-none text-pucho-dark pr-10 hover:border-pucho-purple transition-colors cursor-pointer" onchange="dashboard.updateDivisionsForSelect('attClass', 'attDiv'); dashboard.loadAttendanceStudents()">
                ...
                <div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle">
                    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                        ...
                        <div class="flex flex-wrap gap-4">
                            <select id="marksClass" class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-sm outline-none hover:border-pucho-purple transition-colors" onchange="dashboard.updateDivisionsForSelect('marksClass', 'marksDiv'); dashboard.loadMarksStudents()">
    ...
    saveStudentMarks: async function (e) {
        ...
        try {
            const webhookUrl = window.appConfig?.webhooks?.results || 'https://studio.pucho.ai/api/v1/webhooks/jveOtzEHwa5TdkeaLLpD4';
    ...
    manage_quizzes: function () {
        const classes = ['Playgroup', 'Nursery', 'LKG', 'UKG'];
        const divisions = ['Butterflies', 'Sunflowers', 'Rainbows', 'Stars', 'Daisies', 'Tulips', 'Roses', 'Lilies'];
        ...
        return `<div class="bg-white rounded-[40px] p-8 border border-gray-100 shadow-subtle animate-fade-in font-inter">
                ...
                <div id="createQuizForm" class="hidden mb-10 mt-8 bg-gray-50 p-6 rounded-3xl border border-gray-100 animate-slide-up">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ${this.renderCustomSelect('quizClass', 'Select Class', classes, 'dashboard.updateQuizDivisionsAndSubjects()')}
                        ${this.renderCustomSelect('quizDiv', 'Select Division', [{ value: 'All', text: 'All Divisions' }])}
    ...
    updateQuizDivisionsAndSubjects: function () {
        // Update subjects
        this.updateQuizSubjects();

        // Update divisions
        const clsValue = document.getElementById('quizClass')?.value;
        const divContainer = document.getElementById('quizDiv-options');
        const triggerText = document.getElementById('quizDiv-trigger-text');
        const hiddenInput = document.getElementById('quizDiv');

        if (!clsValue || !divContainer) return;

        const divs = this.classDivisionsMap[clsValue] || [];
        const optionsList = [{ value: 'All', text: 'All Divisions' }, ...divs.map(d => ({ value: d, text: `Division ${d}` }))];

        // Clear selection
        if (hiddenInput) hiddenInput.value = '';
        if (triggerText) {
            triggerText.innerText = 'Select Division';
            triggerText.classList.add('text-gray-400');
            triggerText.classList.remove('text-pucho-dark');
        }

        // Re-render options
        divContainer.innerHTML = optionsList.map(o => `
            <div onclick="dashboard.selectCustomOption('quizDiv', '${o.value}', '${o.text}')" 
                 class="px-5 py-3 hover:bg-pucho-purple/5 hover:text-pucho-purple cursor-pointer transition-colors font-bold text-sm text-gray-600 border-b border-gray-50 last:border-0">${o.text}</div>
        `).join('');
    },
                ...
                <div class="bg-white rounded-[40px] p-8 md:p-10 border border-gray-100 shadow-subtle min-h-[500px]">
                    <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
                        ...
                        <div class="flex flex-wrap gap-4 w-full md:w-auto">
                            <select id="filterHwClass" onchange="dashboard.updateDivisionsForSelect('filterHwClass', 'filterHwDiv', true); dashboard.updateHomeworkList()" class="flex-1 md:flex-none px-6 py-3 rounded-2xl bg-gray-50 border border-gray-200 text-sm font-bold text-pucho-dark outline-none focus:border-pucho-purple cursor-pointer">
                    ...
                    <form class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="hwClass" onchange="dashboard.updateDivisionsForSelect('hwClass', 'hwDivision'); dashboard.updateHwSubjects()" class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-bold text-sm outline-none focus:border-pucho-purple appearance-none">
                                    <option value="">Select Class</option>
                                    ${classes.map(c => `<option value="${c}">${c}</option>`).join('')}
                                </select>
                            </div>
    ...
    uploadHomework: async function () {
        ...
        try {
                ...
                if (students && students.length > 0) {
                    const webhookUrl = window.appConfig?.webhooks?.homework || 'https://studio.pucho.ai/api/v1/webhooks/TV3oIpil0k3BYxeBX5Tni';
                    const payload = {
                ...
                <div id="report-content">
                    <div class="page-header">
                        <div class="pucho-logo">
                            <img src="/images/pucho_logo_login.png" alt="Pucho Logo" class="school-logo-img">
                        </div>
                    </div>
    ...
    runRecoveryFlow: async function () {
        ...
        const webhookUrl = window.appConfig?.webhooks?.feeRecovery || 'https://studio.pucho.ai/api/v1/webhooks/IID0LvlCsMbMXasFGMY6E';
    ...
    // --- PAYROLL & SALARIES HELPER METHODS ---
    ...
    processPayment: function (id) {
        ...
        const payKey = `pay_status_${id}_${currentMonth.replace(' ', '_')}`;
        
        showToast(`Processing payment for ${staff.name}...`, 'info');
        setTimeout(() => {
            localStorage.setItem(payKey, 'paid');
            showToast(`Salary for ${currentMonth} paid to ${staff.name}!`, 'success');
            
            // Trigger Management Dashboard Flow - salary_calculation
            try {
                fetch(window.appConfig?.webhooks?.managementDashboard || 'https://studio.pucho.ai/api/v1/webhooks/ReuxtwNMNxtc7boiNZY5g', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                console.error('Salary webhook error:', e);
            }
    ...
    filterStaffPayroll: function () {
        ...
        body.innerHTML = filtered.map(s => {
            ...
            const allowance = parseFloat(s.special_allowance) || 0;
            
            const gross = basic + hra + conveyance + allowance;
            const epf = Math.round(basic * 0.12);
            const pt = basic > 10000 ? 200 : 0;
            const deductions = epf + pt;
            const net = gross - deductions;

            const payKey = `pay_status_${s.id}_${currentMonth.replace(' ', '_')}`;
            const isPaid = localStorage.getItem(payKey) === 'paid';
                ...
                <tr class="hover:bg-gray-50/50 font-inter animate-fade-in transition-all">
                    <td class="p-6 border-b border-gray-50">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="font-bold text-pucho-dark">${s.name || 'Unknown'}</div>
                                <div class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">${s.role || 'Teacher'}</div>
                            </div>
                        </div>
                    </td>
                    <td class="p-6 border-b border-gray-50 text-sm font-bold text-gray-600">
                        ₹${gross.toLocaleString()}
                        <br><span class="text-[10px] text-gray-400 font-medium">B: ${basic} | H: ${hra} | A: ${allowance}</span>
                    </td>
                    <td class="p-6 border-b border-gray-50 text-sm font-bold text-red-500">
                        ₹${deductions.toLocaleString()}
                        <br><span class="text-[10px] text-gray-400 font-medium">EPF: ${epf} | PT: ${pt}</span>
                    </td>
                    <td class="p-6 border-b border-gray-50 text-sm font-black text-green-600">₹${net.toLocaleString()}</td>
                    <td class="p-6 border-b border-gray-50">
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold ${isPaid ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}">
                            ${isPaid ? '✅ Paid' : '⏳ Pending'}
                        </span>
                    </td>
                    <td class="p-6 border-b border-gray-50">
                        ${isPaid ? 
                            `<button onclick="dashboard.viewPayslip('${s.id}')" class="px-4 py-2 bg-gray-100 hover:bg-pucho-purple hover:text-white rounded-xl text-xs font-bold text-gray-600 transition-all">📄 Payslip</button>` :
                            `<button onclick="dashboard.processPayment('${s.id}')" class="px-4 py-2 bg-pucho-purple text-white hover:bg-pucho-dark rounded-xl text-xs font-bold transition-all">💸 Pay Salary</button>`
                        }
                    </td>
                </tr>
            `;
    ...
    viewPayslip: function (id) {
        ...
        const allowance = parseFloat(staff.special_allowance) || 0;
        ...
        if (!modal) {
            ...
            modal.className = 'fixed inset-0 z-[120] flex items-center justify-center p-4 bg-pucho-dark/40 backdrop-blur-sm animate-fade-in shadow-glow';
            document.body.appendChild(modal);
        }
        modal.classList.remove('hidden');
        ...
        const netInWords = numberToWords(net).toUpperCase();

        modal.innerHTML = `
            <div class="bg-white p-8 w-full max-w-2xl rounded-[32px] border border-white/30 shadow-2xl relative animate-slide-up max-h-[90vh] overflow-y-auto custom-scrollbar">
                <button onclick="document.getElementById('payslipModal').classList.add('hidden')" class="absolute top-6 right-6 p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-pucho-dark font-bold">✕</button>
                
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-bold text-2xl text-pucho-dark">Employee Payslip</h3>
                    <button onclick="dashboard.downloadPayslipPDF('${staff.id}')" class="px-6 py-2.5 bg-pucho-dark text-white rounded-xl text-xs font-black uppercase shadow-sm hover:bg-pucho-purple transition-all">📥 Download PDF</button>
                </div>

                <div id="payslipPrintArea" class="p-6 border border-gray-100 rounded-2xl bg-white font-inter text-sm text-gray-700">
                    <div style="text-align: center; border-bottom: 2px solid #E5E7EB; padding-bottom: 16px; margin-bottom: 20px;">
                        <h2 style="font-size: 20px; font-weight: 800; color: #111834; margin: 0 0 4px 0;">Little Blooms</h2>
                        <p style="font-size: 11px; color: #6B7280; font-weight: 500; margin: 0;">12, Blossom Park, Garden Colony, Pune - 411001 · Phone: +91 98765 43210</p>
                        <h4 style="font-size: 13px; font-weight: 700; color: #8b5cf6; margin: 12px 0 0 0; text-transform: uppercase; letter-spacing: 1.5px;">Payslip for the month of ${currentMonth}</h4>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; font-size: 12px; border-bottom: 1px solid #F3F4F6; padding-bottom: 16px;">
                        <div>
                            <p style="margin: 0 0 6px 0;"><strong style="color: #111834;">Employee ID:</strong> ${staff.id}</p>
                            <p style="margin: 0 0 6px 0;"><strong style="color: #111834;">Employee Name:</strong> ${staff.name}</p>
                            <p style="margin: 0;"><strong style="color: #111834;">Designation:</strong> ${staff.designation || 'Class Teacher'}</p>
                        </div>
                        <div>
                            <p style="margin: 0 0 6px 0;"><strong style="color: #111834;">Class Assigned:</strong> ${staff.class_assigned || 'Playgroup'}</p>
                            <p style="margin: 0 0 6px 0;"><strong style="color: #111834;">Department:</strong> ${staff.dept || 'Academics'}</p>
                            <p style="margin: 0;"><strong style="color: #111834;">Payment Status:</strong> <span style="color: #16A34A; font-weight: 700;">PAID</span></p>
                        </div>
                    </div>

                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 12px;">
                        <thead>
                            <tr style="background-color: #F9FAFB; border-bottom: 1px solid #E5E7EB; border-top: 1px solid #E5E7EB;">
                                <th style="text-align: left; padding: 10px; font-weight: 700; color: #111834; text-transform: uppercase;">Earnings</th>
                                <th style="text-align: right; padding: 10px; font-weight: 700; color: #111834; text-transform: uppercase;">Amount (₹)</th>
                                <th style="text-align: left; padding: 10px; font-weight: 700; color: #111834; text-transform: uppercase; border-left: 1px solid #E5E7EB;">Deductions</th>
                                <th style="text-align: right; padding: 10px; font-weight: 700; color: #111834; text-transform: uppercase;">Amount (₹)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr style="border-bottom: 1px solid #F3F4F6;">
                                <td style="padding: 10px; color: #4B5563;">Basic Salary</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #111834;">${basic.toLocaleString()}</td>
                                <td style="padding: 10px; color: #4B5563; border-left: 1px solid #E5E7EB;">Provident Fund (EPF)</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #EF4444;">${epf.toLocaleString()}</td>
                            </tr>
                            <tr style="border-bottom: 1px solid #F3F4F6;">
                                <td style="padding: 10px; color: #4B5563;">House Rent Allowance (HRA)</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #111834;">${hra.toLocaleString()}</td>
                                <td style="padding: 10px; color: #4B5563; border-left: 1px solid #E5E7EB;">Professional Tax (PT)</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #EF4444;">${pt.toLocaleString()}</td>
                            </tr>
                            <tr style="border-bottom: 1px solid #F3F4F6;">
                                <td style="padding: 10px; color: #4B5563;">Conveyance Allowance</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #111834;">${conveyance.toLocaleString()}</td>
                                <td style="padding: 10px; border-left: 1px solid #E5E7EB;"></td>
                                <td style="text-align: right; padding: 10px;"></td>
                            </tr>
                            <tr style="border-bottom: 1px solid #F3F4F6;">
                                <td style="padding: 10px; color: #4B5563;">Special Allowance</td>
                                <td style="text-align: right; padding: 10px; font-weight: 600; color: #111834;">${allowance.toLocaleString()}</td>
                                <td style="padding: 10px; border-left: 1px solid #E5E7EB;"></td>
                                <td style="text-align: right; padding: 10px;"></td>
                            </tr>
                            <tr style="background-color: #F9FAFB; border-top: 1px solid #E5E7EB; border-bottom: 1px solid #E5E7EB; font-weight: 700;">
                                <td style="padding: 10px; color: #111834;">Gross Earnings</td>
                                <td style="text-align: right; padding: 10px; color: #111834;">₹${gross.toLocaleString()}</td>
                                <td style="padding: 10px; color: #111834; border-left: 1px solid #E5E7EB;">Total Deductions</td>
                                <td style="text-align: right; padding: 10px; color: #EF4444;">₹${deductions.toLocaleString()}</td>
                            </tr>
                        </tbody>
                    </table>

                    <div style="border: 2px solid #E5E7EB; border-radius: 12px; padding: 16px; background-color: #F9FAFB; display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                        <div>
                            <span style="font-size: 11px; text-transform: uppercase; color: #6B7280; font-weight: 700; display: block; margin-bottom: 2px;">Net Pay (In Words)</span>
                            <span style="font-weight: 700; color: #4B5563; font-size: 11px; line-height: 1.4;">${netInWords}</span>
                        </div>
                        <div style="text-align: right;">
                            <span style="font-size: 11px; text-transform: uppercase; color: #6B7280; font-weight: 700; display: block; margin-bottom: 2px;">Net Salary Pay</span>
                            <span style="font-size: 20px; font-weight: 900; color: #8b5cf6;">₹${net.toLocaleString()}</span>
                        </div>
                    </div>

                    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: 40px; font-size: 11px; color: #4B5563;">
                        <div style="text-align: center;">
                            <div style="width: 120px; border-bottom: 1px solid #D1D5DB; margin-bottom: 8px;"></div>
                            <p style="margin: 0; font-weight: 500;">Employee Signature</p>
                        </div>
                        <div style="text-align: center;">
                            <div style="width: 120px; border-bottom: 1px solid #D1D5DB; margin-bottom: 8px; font-weight: 700; color: #111834; text-align: center;">Little Blooms HR</div>
                            <p style="margin: 0; font-weight: 500;">Authorized Signatory</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    downloadPayslipPDF: function (id) {
    ...
    triggerManagementFlow: async function (action) {
        showToast(`Triggering ${action.replace('_', ' ')}...`, 'info');
        try {
            const res = await fetch(window.appConfig?.webhooks?.managementDashboard || 'https://studio.pucho.ai/api/v1/webhooks/ReuxtwNMNxtc7boiNZY5g', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    action: action,
            ...
            if (res.ok) {
                showToast(`✅ ${action.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} flow triggered!`, 'success');
            } else {
                showToast(`❌ Flow trigger failed: ${res.status}`, 'error');
            }
        } catch (e) {
            console.error('Management flow error:', e);
            showToast('❌ Connection error triggering flow', 'error');
        }
    }
};
